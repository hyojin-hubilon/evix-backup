        ...
        const handleVerifyInviteToken = async (token: string) => {
            try {
                ...
                if (response.code === 200) {
					...
					confirm({
						description: response.message ? response.message : t('auth.study_invitation_completed'),//"스터디 초대가 완료되었습니다."
						variant: 'info'
					})
					.then(() => { 
						navigate('/study');
                ...
                if (response.code === 404) {
					...
					confirm({
						description: response.message ? response.message : t('auth.register_member_first'),//"회원가입을 먼저 진행해주십시오.",
						variant: 'info'
					})
					.then(() => { 
                ...
                if (response.code === 208 || response.code === 401) {
                    ...
                    // 401 : 토큰 유효시간 만료
					confirm({
						description: response.message ? response.message : t('auth.token_expired'),//"토큰이 만료되었습니다.",
						variant: 'info'
					})
					.then(() => { 
						navigate('/');
                ...
                if (e) {
					confirm({
						description: e.message ? e.message : t('auth.error_occurred'),//에러가 발생했습니다.
						variant: 'info'
					})
					.then(() => { 
						navigate('/');
