...
const MdppSurvey = () => {
            ...
            .of(
                Yup.object().shape({
                    ...
                    answer: Yup.mixed().when(['isRequired', 'questionType'], {
                        is: (isRequired: boolean, questionType: string) =>
                            isRequired &&
                            (questionType == QuestionTypes.RADIO ||
                                questionType == QuestionTypes.WRITE),
                        then: (s) => s.required('필수항목 입니다.'),
                        otherwise: (s) => s.notRequired(),
                    }),
                    answerMultiple: Yup.array().when(['isRequired', 'questionType'], {
                        is: (isRequired: boolean, questionType: string) =>
                            isRequired && questionType == QuestionTypes.MULTIPLE,
    ...
    const getSurveyDeatil = async () => {
        try {
            const response = await participantSurveyApi.getSurveyDetail(
                setNo,
                surveyNo,
                surveyCycle,
                surveyTurn
            ...
            if (response.result && response.code === 200) {
                ...
                if (survey.questionList) {
                    const hasRequiredCheck = survey.questionList.some(
                        (card) => card.required_answer_yn === 'Y'
                    );
    ...
    const handleSumbit = (values: InitialValuesType) => {
        ...
        values.questions.forEach((value, index) => {
            ...
            if (
                value.questionType === QuestionTypes.RADIO ||
                value.questionType === QuestionTypes.SINGLE
            ) {
            ...
            if (value.questionType == QuestionTypes.MULTIPLE) {
                answerSelect = value.answerMultiple.reduce(
                                ...
                                <FieldArray
                                    ...
                                    render={() => {
                                        return (
                                            <>
                                                {previewCards &&
                                                    previewCards.map((card, index: number) => (
                                                        <ViewCard
                                                            key={index}
                                                            index={index}
                                                            card={card}
