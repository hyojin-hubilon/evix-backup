...
import {
    ParticipantStudyDetail,
    ParticipantSurveySet,
...
const ParticipantSurveys = () => {
    ...
    const [studyDetail, setStudyDetail] = useState<ParticipantStudyDetail>(
        {} as ParticipantStudyDetail
    );
    ...
    const handleSelectSurvey = (surveySet: SurveySetList) => {
        ...
        navigate(
            `/mdpp/survey/${surveySet.set_no}/${surveySet.survey_no}/${surveySet.answer_cycle}/${surveySet.answer_turn}`
        );
        ...
        <Box sx={{ bgcolor: 'white', minHeight: '100vh', pt: '22px' }}>
            ...
            <Box p="24px 23px">
                <S.BlueBox>
                    ...
                    <S.CommonText>{studyDetail?.description}</S.CommonText>
                </S.BlueBox>
            </Box>

            <Box p="0 23px">
                {surveySets.map((survey, index) => (
                    <ParticipantSurveyItem
                        survey={survey}
                        key={index}
                        study={studyDetail}
                        selectSurvey={handleSelectSurvey}
                    />
                ))}
            </Box>
        </Box>
