...
const ParticipantStudyItem = ({ study }: ParticipantStudyItemType) => {
    ...
    const signature_eic_name = false;
    const toEicPage = () => {
        if (undefined) {
            return;
        }
        // navigate(`/mdpp/eic/${study.}`);
    };
    ...
    return (
        <Box p="20px 25px" borderBottom="1px solid #E0E5E9" position="relative">
            ...
            <Box pt="14px">
                <S.StudyDetail>
                    참여기간: {study.start_date ? dayjs(study.start_date).format('YYYY.MM.DD') : ''}{' '}
                    ~ {study.end_date ? dayjs(study.end_date).format('YYYY.MM.DD') : ''}{' '}
                </S.StudyDetail>
                <S.StudyDetail>참여기관: {study.organization}</S.StudyDetail>
            </Box>
            <Box display="flex" alignItems="center" gap="5px" mt="15px">
                <S.StudyTags>
                    {surveyCycle[study.survey_cycle]} {study.number_in_cycle} 회
                </S.StudyTags>
                <S.StudyTags>
                    {study.number_of_participation == 0
                        ? '참여대기중'
                        : `총 ${study.number_of_participation}회 참여`}
                </S.StudyTags>
            </Box>
            <Box
                sx={{
            ...
            >
                ...
                >
                    <path
                        ...
                        stroke={`${signature_eic_name ? `#AFB3BA` : `#000001`}`}
                        strokeLinecap="round"
