...
const AllPartTransitionChart = ({ title } : {title:string}) => {
	...
	useEffect(() => {
		// 테마 또는 데이터 또는 slot이 변경될 때마다 실행됩니다.
		setOptions((prevState) => ({
			...prevState,
			colors: [theme.palette.primary.main], // 차트 색상을 설정합니다.
			xaxis: {
				type: 'datetime',
				min: getMinDate(),//가져온 데이터 중 맨 처음 날짜
				tickAmount: 6,
				labels: {
					style: {
						colors: [secondary] // x축 레이블의 색상을 설정합니다.
					}
				},
				axisBorder: {
					show: true,
					color: line // x축 경계선의 색상을 설정합니다.
				}
			},
			yaxis: {
				labels: {
	...
	}, [slot]); // 종속성 배열: slot이 변경될 때마다 실행됩니다.
