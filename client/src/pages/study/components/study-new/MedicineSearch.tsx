...
import { Box, Button, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, FormControl, Grid, MenuItem, OutlinedInput, Select, SelectChangeEvent, TableContainer, Typography, useTheme } from "@mui/material";
...
const MedicineSearch = ({isOpen, handleClose, selectMedicine}: MedicineSearchProps) => {
	...
	const columns : GridColDef[] = [
		...
		{ field: 'productName', headerName: '제품명', width: 400,
			renderCell: (params) => {
				const onClickName = (e) => {
				...
				return <span style={{textDecoration:'underline', color: primary.main, fontWeight:600, cursor: 'pointer' }} onClick={onClickName}>{ params.value }</span>;
			}
		},
						...
						<Grid item xs={8}>
							<form onSubmit={(e)=> handleSearch(e)}>
								<Box gap={1} display="flex">
									<FormControl size="small" fullWidth>
										<OutlinedInput placeholder="검색어 입력" onChange={(e) => setSearchTerm(e.target.value)} />
									</FormControl>
									<Button type="submit"variant="contained" color="primary" onClick={handleSearch}>검색</Button>	
								</Box>
							</form>
						</Grid>
					</Grid>
